...
- **Attach a photo or file to one message** — the paperclip in the composer (or dragging onto it) lets you ask about a specific image or document for that message only, without adding it to the permanent knowledge base above. Docs get their text read and included; photos are sent as an image, which only works if the model you've picked supports vision (most of the defaults here don't).
...
const BRAND_COLOR       = '#9C27B0';  // default brand color — dark mode (purple)
const BRAND_COLOR_LIGHT = '#EC4899';  // default brand color — light mode (pink)
